...
\author[1,$\ast$]{Shuaiyao Fan}
%\author[1]{Coauthor Name}

\address[1]{\orgdiv{School/Department to be added}, \orgname{China University of Geosciences (Beijing)}, \orgaddress{\city{Beijing}, \country{China}}}

\corresp[$\ast$]{Corresponding author. \href{mailto:your-email@example.com}{your-email@example.com}}

\abstract{
\textbf{Motivation:} Molecular property prediction benefits from multimodal learning because molecular graphs, line notations, fingerprints and textual descriptions provide complementary evidence. However, existing multimodal frameworks often rely on noisy molecular text and unstable many-to-many alignment across modalities, which can limit both representation quality and downstream predictive performance.\\
\textbf{Results:} We present KGAMA, a knowledge-enhanced graph-anchor multimodal alignment framework for molecular property prediction. KGAMA integrates four modalities, including molecular graphs, SMILES sequences, molecular fingerprints and rewritten scientific text. At the data level, we construct a knowledge enhancement pipeline that cleans PubChem descriptions with a large language model and injects 11 RDKit-derived physicochemical descriptors to increase textual density and constrain semantic drift. At the representation level, we adopt graph-centered contrastive pretraining, where the molecular graph acts as the semantic anchor for cross-modal alignment, and combine it with uncertainty-aware adaptive weighting to stabilize multimodal optimization. During downstream finetuning, cross-attention is used to fuse modality-specific representations for task prediction. Experiments on 9 MoleculeNet benchmarks show that KGAMA achieves the best overall performance among competitive baselines, reaching AUROC values of 0.872 on BACE, 0.930 on BBBP, 0.939 on ClinTox, 0.841 on Tox21, 0.712 on ToxCast and 0.669 on SIDER, together with RMSE values of 0.831 on ESOL, 1.512 on FreeSolv and 0.655 on Lipophilicity. Ablation, visualization and parameter sensitivity analyses further support the contribution of knowledge-enhanced text, graph-centered alignment and dynamic multimodal fusion.\\
...
The text branch provides semantic knowledge about molecular properties and biological effects. To improve its reliability, KGAMA introduces a knowledge-enhanced rewriting pipeline. For each molecule, we first retrieve raw textual descriptions from PubChem and compute 11 physicochemical descriptors with RDKit, including features such as LogP and topological polar surface area. We then use a large language model to rewrite the original description under three constraints: domain-expert role conditioning, joint use of raw text and quantitative descriptors, and explicit suppression of irrelevant content such as patent identifiers, historical anecdotes and commercial names. The rewritten text is subsequently encoded with SciBERT, which is well suited to scientific language and produces dense task-relevant semantic representations.
...
We compared KGAMA with representative baselines from several categories, including conventional molecular representation methods, graph neural networks, self-supervised molecular encoders and multimodal pretraining frameworks. Across the 9 MoleculeNet tasks, KGAMA achieved the strongest overall performance. The classification results are summarized in Table~\ref{tab:classification-results}, and the regression results are summarized in Table~\ref{tab:regression-results}.

On classification benchmarks, KGAMA reached AUROC values of 0.872 on BACE, 0.930 on BBBP, 0.939 on ClinTox, 0.841 on Tox21, 0.712 on ToxCast and 0.669 on SIDER. These results consistently surpassed or matched strong graph-based competitors such as AttentiveFP, MPNN, DMPNN, CMPNN and GIN. The gains were especially notable on ClinTox and SIDER, where pharmacological and toxicological knowledge is important, suggesting that the knowledge-enhanced text branch contributes complementary information beyond structural encodings alone.
...
To better interpret modality usage across tasks, we analysed the learned attention weights assigned to graph, sequence, fingerprint and text branches. The learned weighting patterns were strongly task dependent and were consistent with chemical intuition, as shown in Figure~\ref{fig:modality-weights}.

For physicochemical regression tasks such as ESOL and FreeSolv, the graph modality dominated, indicating that the structural topology extracted by Graphormer provides the most informative signal for solubility and solvation-related properties. In contrast, the fingerprint branch received the highest contribution on Lipophilicity, which is consistent with the dependence of lipophilicity on the occurrence of characteristic hydrophobic and hydrophilic functional groups.

For biologically complex tasks such as SIDER, ClinTox and Tox21, the text modality received a marked increase in attention weight. This pattern suggests that the rewritten and descriptor-enriched text branch successfully captures pharmacological semantics that are difficult to infer from structure alone. The relative stability of the SMILES branch across tasks indicates that sequence features act as a robust complementary view that supports multimodal integration without dominating it.
