...
The detailed knowledge-rewriting procedure is presented in Algorithm~\ref{alg:knowledge_rewriting}. The algorithm accepts a SMILES string and PubChem CID as input, then queries the PubChem PUG View API to obtain raw textual descriptions. It extracts ten physicochemical descriptors using RDKit, constructs specific prompts for the LLM, and generates an enhanced description with integrated molecular properties. An optional validation step ensures descriptor consistency in the output.
